...
_BLOCKED_EXTENSIONS: frozenset[str] = frozenset({
    ".exe", ".sh", ".bat", ".cmd", ".com", ".scr", ".ps1", ".msi",
    ".dll", ".jar", ".app", ".pkg",
})
    ...
    async def get_download_url(
        ...
        *,
        ...
        media = await MediaService.get_by_id(
            db, institution_id=institution_id, media_id=media_id
        )
    ...
    *,
    ...
    if str(media.uploader_id) != str(uploader_id):
        raise PermissionDeniedError(
            "You can only modify media you uploaded."
        )
    return media
...
def _build_storage_key(
    *,
    ...
    ext = PurePosixPath(file_name).suffix.lower()
    return (
        f"{institution_id}/originals/{now.year:04d}/"
        f"{now.month:02d}/{media_id}{ext}"
    )
