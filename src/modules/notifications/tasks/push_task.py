...
from src.modules.notifications.celery_app import celery_app
...
def send_push_batch(self, batch_id: str, institution_id: str) -> dict[str, Any]:  # type: ignore[no-untyped-def]  # pragma: no cover - Celery entry point
    ...
    try:
        ...
        CELERY_TASK_LATENCY.labels(task_name=_TASK_NAME).observe(
            time.monotonic() - start
        )


async def _run_batch(batch_id: str, institution_id: str) -> dict[str, Any]:  # pragma: no cover - driven by Celery worker only
...
async def _process_batch(
    ...
    notification = await db.scalar(
        select(Notification).where(
            Notification.id == batch.notification_ids[0]
        )
    )
    ...
    tokens = await TokenService.list_active_tokens(
        db, user_id=uuid.UUID(batch.user_id)
    )
    if not tokens:
        await FanoutService.record_batch_result(
            db,
    ...
    for stale in response.unregistered_tokens:
        await TokenService.deactivate_by_string(
            db, institution_id=institution_id, token=stale
        )
