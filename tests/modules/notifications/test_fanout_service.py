...
@pytest_asyncio.fixture
async def recipients(db_session: AsyncSession, institution: Institution) -> list[User]:
    return [
        await make_user(db_session, institution=institution, email=f"r{i}@x.test") for i in range(3)
    ]
...
class TestDispatchMessageNotifications:
    ...
    async def test_persists_notifications_and_batches_and_enqueues(
        ...
        notif_count = await db_session.scalar(text("SELECT count(*) FROM notifications"))
    ...
    async def test_unicode_title_and_body_survives_round_trip(
        ...
        persisted = await db_session.scalar(
            select(Notification).where(Notification.id == batches[0].notification_ids[0])
        )
