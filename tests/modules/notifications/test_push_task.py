...
class TestRunBatch:
    ...
    async def test_success_marks_batch_sent_and_increments_metric(
        ...
        refreshed = await db_session.scalar(
            select(NotificationBatch).where(
                NotificationBatch.id == batch_with_token.id
            )
        )
    ...
    async def test_unregistered_tokens_are_deactivated(
        ...
        token_row = await db_session.scalar(
            select(FcmToken).where(FcmToken.user_id == bob.id)
        )
        ...
        refreshed = await db_session.scalar(
            select(FcmToken).where(FcmToken.id == token_row.id)
        )
        assert refreshed is not None
...
class TestDefaultFcmClient:
    def test_default_client_reports_all_success(self) -> None:
        ...
        resp = client.send_multicast(
            tokens=["a", "b", "c"], title="t", body="b", data={"k": "v"}
        )
