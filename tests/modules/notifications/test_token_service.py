...
class TestRegisterToken:
    ...
    async def test_same_user_same_token_is_idempotent(
        ...
        count = await db_session.scalar(
            text(
                "SELECT count(*) FROM fcm_tokens WHERE user_id = :uid"
            ),
            {"uid": alice.id},
        )
        assert count == 1
...
class TestRevokeToken:
    ...
    async def test_revoke_own_token_deactivates_and_audits(
        ...
        refreshed = await db_session.scalar(
            select(FcmToken).where(FcmToken.id == row.id)
        )
...
class TestCrossTenantFuzz:
    @pytest.mark.asyncio
    async def test_cross_institution_revoke_returns_404(
...
class TestListActiveTokens:
    ...
    async def test_returns_only_active_for_user(
        ...
        active = await TokenService.list_active_tokens(
            db_session, user_id=uuid.UUID(alice.id)
        )
