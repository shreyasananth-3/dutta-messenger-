...
class TestUtcNow:
    ...
    def test_returns_aware_utc(self) -> None:
        ...
        assert now == datetime(2026, 4, 18, 12, 0, 0, tzinfo=timezone.utc)
...
class TestIsExpired:
    ...
    def test_past_time_is_expired(self) -> None:
        past = datetime(2026, 4, 18, 11, 0, tzinfo=timezone.utc)
        assert is_expired(past) is True

    @freeze_time("2026-04-18T12:00:00+00:00")
    def test_future_time_not_expired(self) -> None:
        future = datetime(2026, 4, 19, 12, 0, tzinfo=timezone.utc)
...
class TestAdders:
    ...
    def test_add_hours_default_now(self) -> None:
        assert add_hours(hours=2) == datetime(2026, 4, 18, 14, 0, tzinfo=timezone.utc)

    def test_add_hours_explicit_dt(self) -> None:
        base = datetime(2026, 1, 1, tzinfo=timezone.utc)
        assert add_hours(base, hours=24) == datetime(2026, 1, 2, tzinfo=timezone.utc)
...
class TestFormatDatetime:
    def test_with_time(self) -> None:
        dt = datetime(2026, 4, 18, 9, 5, 7, tzinfo=timezone.utc)
...
class TestGetTimeUntil:
    ...
    def test_target_in_past_returns_zeros(self) -> None:
        past = datetime(2026, 4, 17, tzinfo=timezone.utc)
        assert get_time_until(past) == {"days": 0, "hours": 0, "minutes": 0, "seconds": 0}

    @freeze_time("2026-04-18T12:00:00+00:00")
    def test_breakdown_correct(self) -> None:
        target = datetime(2026, 4, 19, 15, 30, 45, tzinfo=timezone.utc)
